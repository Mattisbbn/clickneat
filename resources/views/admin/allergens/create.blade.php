<x-admin-layout>
    ...
    <form class="d-flex flex-column justify-content-center align-items-center" method="post" action="{{ route("allergens.store") }}">
        @csrf
        @method("post")
        <div class="d-flex flex-column bg-white shadow p-4 rounded-3">
            <h1 class="text-center mb-4">Créer un allergène</h1>
            ...
            <select class="p-2 rounded-3 border-0 mb-4 shadow-sm" name="item_id">
                @foreach($items as $item)
                    <option value="{{ $item->id }}">{{ $item->name }} | {{ $item->category->name }} | {{ $item->category->restaurant->name }}</option>
                @endforeach
            </select>
